...
openssl req -new -newkey rsa:4096 \
    ...
    -subj "/C=US/ST=Minnesota/L=Minneapolis/O=Localhost/OU=Testing/CN=xago.local" \
    -addext "subjectAltName=DNS:localhost,DNS:*.localhost,DNS:xago.local,IP:127.0.0.1"

openssl req -new -newkey rsa:4096 \
    -nodes -keyout counterparty.key.pem -out counterparty.csr \
    -subj "/C=DE/ST=Hesse/L=Frankfurt/O=Counterparty/OU=Testing/CN=luno.local" \
    -addext "subjectAltName=DNS:localhost,DNS:*.localhost,DNS:luno.local,IP:127.0.0.1"
